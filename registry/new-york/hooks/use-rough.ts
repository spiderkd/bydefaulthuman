...
// Registry builds need a standalone hook that does not depend on app context.
// Defaults chosen to match the registry's intended static output.
export function useRough({
	...
	// Registry defaults (no CrumbleContext available here)
	const animateOnHover = true;
	const animateOnMount = true;
	const theme: CrumbleTheme = themeProp ?? "pencil";
