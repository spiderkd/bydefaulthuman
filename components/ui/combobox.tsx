...
export function Combobox({
  ...
  const comboId = id ?? `combobox-${label?.toLowerCase().replace(/\s+/g, "-") ?? "field"}`;

  const filtered = query.trim()
    ? options.filter((o) => o.label.toLowerCase().includes(query.toLowerCase()))
    : options;

  const drawTrigger = useCallback((reseed = false) => {
    ...
    svg.setAttribute("viewBox", `0 0 ${w} ${TRIGGER_HEIGHT}`);
    ...
    svg.appendChild(rc.rectangle(1, 1, w - 2, TRIGGER_HEIGHT - 2, getRoughOptions(theme, "border", {
      fill: "none",
  ...
  const drawDropdown = useCallback((svg: SVGSVGElement, w: number, h: number) => {
    svg.replaceChildren();
    svg.setAttribute("width", String(w));
  ...
  const drawOption = useCallback((svg: SVGSVGElement, w: number, highlighted: boolean) => {
    svg.replaceChildren();
    svg.setAttribute("width", String(w));
    svg.setAttribute("height", String(OPTION_HEIGHT));
    svg.setAttribute("viewBox", `0 0 ${w} ${OPTION_HEIGHT}`);
    if (!highlighted) return;
    const rc = rough.svg(svg);
    svg.appendChild(rc.rectangle(2, 2, w - 4, OPTION_HEIGHT - 4, getRoughOptions(theme, "fill", {
      fill: "currentColor",
      fillStyle: "hachure",
  ...
  useEffect(() => { drawTrigger(); }, [drawTrigger]);

  useEffect(() => {
    const wrapper = wrapperRef.current;
    if (!wrapper) return;
    const ro = new ResizeObserver(() => drawTrigger());
    ro.observe(wrapper);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (wrapperRef.current && !wrapperRef.current.contains(e.target as Node)) {
        setOpen(false); setFocused(false); setQuery(""); onBlur?.();
      }
    };
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      {label ? (
        <label htmlFor={comboId} className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
          {label}
        </label>
      ) : null}
      ...
      <div ref={wrapperRef} className="relative">
        ...
        <div className="relative" style={{ height: TRIGGER_HEIGHT }}>
          ...
          />
          {/* Chevron */}
          <svg aria-hidden="true" width="16" height="14" viewBox="0 0 16 14"
        ...
        {open ? (
          ...
          >
            <svg
              ...
              ref={(el) => {
                dropdownSvgRef.current = el;
                if (el) {
                  const w = el.parentElement?.offsetWidth ?? 200;
                  const h = filtered.length * OPTION_HEIGHT;
                  drawDropdown(el, w, h || OPTION_HEIGHT);
                }
              }}
            />
            {filtered.length === 0 ? (
              <div className="flex items-center px-3 text-sm text-muted-foreground" style={{ height: OPTION_HEIGHT }}>
                No results
              </div>
            ) : filtered.map((opt) => {
              ...
              return (
                <div
                  ...
                  onMouseEnter={(e) => {
                    const svg = optionSvgRefs.current.get(opt.value);
                    if (svg) drawOption(svg, (e.currentTarget as HTMLElement).offsetWidth, true);
                  }}
                  onMouseLeave={(e) => {
                    const svg = optionSvgRefs.current.get(opt.value);
                    if (svg) drawOption(svg, (e.currentTarget as HTMLElement).offsetWidth, false);
                  }}
                >
                  <svg
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-0 overflow-visible"
                    ref={(el) => {
                      if (el) optionSvgRefs.current.set(opt.value, el);
                      else optionSvgRefs.current.delete(opt.value);
                    }}
                  />
