...
import { StickyNote, type StickyNoteColor } from "@/registry/new-york/ui/sticky-note";
    ...
    tasks: [
      {
        ...
        description: "Align install command copy with the current registry path.",
        priority: "low",
        assigneeFallback: "BM",
        tag: "site",
      },
...
export function KanbanColumnBlock({
  ...
  const updateBoard = (updater: (current: KanbanColumn[]) => KanbanColumn[]) => {
    setColumns((current) => {
        ...
        column.id === editingTask.columnId
          ? {
              ...
              tasks: column.tasks.map((task) =>
                task.id === editingTask.taskId ? { ...task, title: value } : task,
              ),
            }
          : column,
      ),
    ...
    <div className="flex w-full flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          />
          <Select
            onChange={(value) => setPriorityFilter(value as "all" | KanbanTask["priority"])}
            options={[
          ...
          return (
            <Card key={column.id} padding={18} style={{ overflow: "visible" }}>
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <Button
                  onClick={() =>
                    setOpenFormColumnId((current) => (current === column.id ? null : column.id))
                  }
                  size="sm"
                  variant="ghost"
                >
                ...
                <div className="mb-4 flex flex-col gap-3">
                  ...
                  <Textarea
                    ...
                    onChange={(event) =>
                      setDrafts((current) => ({
                        ...current,
                        [column.id]: { ...draft, description: event.target.value },
                      }))
                    }
                  ...
                  <Select
                    onChange={(value) =>
                      setDrafts((current) => ({
                        ...current,
                        [column.id]: { ...draft, priority: value as KanbanTask["priority"] },
                      }))
                    }
                    options={[
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-3">
                        {task.description ? (
                          <p className="text-xs leading-5 text-foreground/80">{task.description}</p>
                        ) : null}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {task.assigneeFallback ? (
                            <Avatar fallback={task.assigneeFallback} size={24} />
                          ) : null}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                          {columns
                            ...
                            .map((targetColumn) => (
                              <Tooltip key={targetColumn.id} content={`Move to ${targetColumn.label}`}>
                                <span>
                                  <Button
                                    onClick={() => moveTask(task.id, column.id, targetColumn.id)}
                                    size="sm"
                                    variant="ghost"
                                  >
                                    {targetColumn.label}
